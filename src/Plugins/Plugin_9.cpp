...
void Plugin_9::set_parameters(uint8_t row)
{
    ...
    if (!neotrellisPressed[TRELLIS_BUTTON_SHIFT])
    {
        if (row == 0)
        {
            ...
            set_voice1_waveform(1, 0);
            set_voice1_detune(2, 0);
            set_voice_amplitude(3, 0);
        }
        ...
        if (row == 2)
        {
            ...
            set_filter_resonance(1, 2, 0.00, MAX_RESONANCE);
            set_filter_sweep(2, 2, 0.00, MAX_FILTERSWEEP);
            set_filter_type(3, 2);
        }
...
void Plugin_9::set_voice_waveform(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_voice_amplitude(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
        float ampl = get_Potentiometer(XPos, YPos) / MIDI_CC_RANGE_FLOAT;

        waveform.amplitude(1.00 - ampl);
        waveform1.amplitude(ampl);
    }
}
void Plugin_9::set_voice_detune(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
        int8_t value = get_Potentiometer(XPos, YPos);
        detune1 = (float)((note_frequency[value] * 4.40));
        if (value == 0)
            detune1 = 0;
    }
}
void Plugin_9::set_voice1_waveform(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
        int walveform = map(get_Potentiometer(XPos, YPos), 0, MIDI_CC_RANGE, 0, 12);
...
void Plugin_9::set_voice1_amplitude(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
        float ampl = get_Potentiometer(XPos, YPos) / MIDI_CC_RANGE_FLOAT;
...
void Plugin_9::set_voice1_detune(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
        detune2 = (float)((note_frequency[get_Potentiometer(XPos, YPos)] * 4.40));
    }
}

void Plugin_9::set_LFO_waveform(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_LFO_frequency(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_LFO_amplitude(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_DC_amplitude(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_filter_frequency(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
        int frequency = note_frequency[get_Potentiometer(XPos, YPos)] * tuning;
        filter.frequency(frequency);
    }
}
void Plugin_9::set_filter_resonance(uint8_t XPos, uint8_t YPos, float min, float max)
{
    if (enc_moved[XPos])
    {
        float reso = get_Potentiometer(XPos, YPos) / (MIDI_CC_RANGE_FLOAT / max) + min;

        filter.resonance(reso);
    }
}
void Plugin_9::set_filter_sweep(uint8_t XPos, uint8_t YPos, float min, float max)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_filter_type(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
        selectFilterType(map(get_Potentiometer(XPos, YPos), 0, MIDI_CC_RANGE, 0, 2));
    }
}
...
void Plugin_9::set_envelope_attack(uint8_t XPos, uint8_t YPos, int min, int max)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_envelope_decay(uint8_t XPos, uint8_t YPos, int min, int max)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_envelope_sustain(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_envelope_release(uint8_t XPos, uint8_t YPos, int min, int max)
{
    if (enc_moved[XPos])
    {
...
void Plugin_9::set_voice2_detune(uint8_t XPos, uint8_t YPos)
{
    if (enc_moved[XPos])
    {
        detune2 = (float)((note_frequency[get_Potentiometer(XPos, YPos)] * 0.01));
    }
}
